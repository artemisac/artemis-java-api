...
/**
 * <b>NMSMaterial</b> - Data Values/Pre-flattening<br>
 ...
 * NMSMaterial v1: https://www.spigotmc.org/threads/329630/
 *
 * @author Crypto Morin
 * @version 4.0.0
 * @see Material
...
public enum NMSMaterial {
    ...
    /**
     * Cached array of {@link NMSMaterial#values()} to avoid allocating memory for
     * calling the method every time.
     *
     * @since 2.0.0
     */
    public static final NMSMaterial[] VALUES = values();
     ...
     * @since 5.1.0
     */
    private static final Map<String, NMSMaterial> NAMES = new HashMap<>();

    /**
     * Guava (Google Core Libraries for Java)'s cache for performance and timed caches.
     * For strings that match a certain NMSMaterial. Mostly cached for configs.
     *
     * @since 1.0.0
     */
     ...
     *
     * @see #matchNMSMaterialWithData(String)
     * @since 8.0.0
     */
    ...
    /**
     * <b>NMSMaterial Paradox (Duplication Check)</b>
     * <p>
    ...
    static {
        for (NMSMaterial material : VALUES) NAMES.put(material.name(), material);
    }
    ...
    NMSMaterial(int data, int version,  String... legacy) {
        this.data = (byte) data;
    ...
    NMSMaterial(int data,  String... legacy) {
        this(data, 0, legacy);
    }

    NMSMaterial(int version) {
        this(0, version);
    }
    ...
    /**
     * Gets the NMSMaterial with this name similar to {@link #valueOf(String)}
    ...
    private static Optional<NMSMaterial> getIfPresent( String name) {
     ...
     *
     * @see #matchDefinedNMSMaterial(String, byte)
     * @since 1.0.0
     */
    
    private static NMSMaterial requestOldNMSMaterial( String name, byte data) {
        String holder = name + data;
        NMSMaterial cache = NAME_CACHE.getIfPresent(holder);
        if (cache != null) return cache;

        for (NMSMaterial material : VALUES) {
    ...
    /**
     * Parses the given material name as an NMSMaterial with a given data
     * value in the string if attached. Check {@link #matchNMSMaterialWithData(String)} for more info.
     *
     * @see #matchNMSMaterialWithData(String)
     * @see #matchDefinedNMSMaterial(String, byte)
     * @since 2.0.0
    ...
    public static Optional<NMSMaterial> matchNMSMaterial( String name) {
        Validate.notEmpty(name, "Cannot match a material with null or empty material name");
        Optional<NMSMaterial> oldMatch = matchNMSMaterialWithData(name);
        return oldMatch.isPresent() ? oldMatch : matchDefinedNMSMaterial(format(name), UNKNOWN_DATA_VALUE);
    }
     ...
     * @return the parsed NMSMaterial.
     * @see #matchNMSMaterial(String)
     * @since 3.0.0
     */

    private static Optional<NMSMaterial> matchNMSMaterialWithData( String name) {
        ...
        if (index != -1) {
            ...
            try {
                ...
                return data >= 0 && data < MAX_DATA_VALUE ? matchDefinedNMSMaterial(mat, data) : matchDefinedNMSMaterial(mat, UNKNOWN_DATA_VALUE);
            } catch (NumberFormatException ignored) {
                return matchDefinedNMSMaterial(mat, UNKNOWN_DATA_VALUE);
            }
        }
    ...
    /**
     * Parses the given material as an NMSMaterial.
     *
     * @throws IllegalArgumentException may be thrown as an unexpected exception.
     * @see #matchDefinedNMSMaterial(String, byte)
     * @see #matchNMSMaterial(ItemStack)
     * @since 2.0.0
     */

    public static NMSMaterial matchNMSMaterial( Material material) {
        Objects.requireNonNull(material, "Cannot match null material");
        return matchDefinedNMSMaterial(material.name(), UNKNOWN_DATA_VALUE)
    ...
    public static NMSMaterial matchXMaterial( Material material) {
        return matchNMSMaterial(material);
    }

    /**
     * Parses the given item as an NMSMaterial using its material and data value (durability)
     * if not a damageable item {@link ItemStack#getDurability()}.
     *
     * @param item the ItemStack to match.
     * @return an NMSMaterial if matched any.
     * @throws IllegalArgumentException may be thrown as an unexpected exception.
     * @see #matchNMSMaterial(Material)
     * @since 2.0.0
     */

    @SuppressWarnings("deprecation")
    public static NMSMaterial matchNMSMaterial( ItemStack item) {
        ...
        return matchDefinedNMSMaterial(material, data)
                .orElseThrow(() -> new IllegalArgumentException("Unsupported material from item: " + material + " (" + data + ')'));
    }

    /**
     * The main method that parses the given material name and data value as an NMSMaterial.
     ...
     * @return an NMSMaterial (with the same data value if specified)
     * @see #matchNMSMaterial(Material)
     * @see #matchNMSMaterial(int, byte)
     * @see #matchNMSMaterial(ItemStack)
     * @since 3.0.0
     */

    protected static Optional<NMSMaterial> matchDefinedNMSMaterial( String name, byte data) {
        ...
        if (Data.ISFLAT || (data <= 0 && !(duplicated = isDuplicated(name)))) {
            Optional<NMSMaterial> NMSMaterial = getIfPresent(name);
            if (NMSMaterial.isPresent()) return NMSMaterial;
        }
        // Usually flat versions wouldn't pass this point, but some special materials do.

        NMSMaterial oldNMSMaterial = requestOldNMSMaterial(name, data);
        if (oldNMSMaterial == null) {
            // Special case. Refer to FILLED_MAP for more info.
            return data > 0 && name.endsWith("MAP") ? Optional.of(FILLED_MAP) : Optional.empty();
        }

        if (!Data.ISFLAT && oldNMSMaterial.isPlural() && (duplicated == null ? isDuplicated(name) : duplicated)) return getIfPresent(name);
        return Optional.of(oldNMSMaterial);
    }
    ...
    private static boolean isDuplicated( String name) {
        // Don't use matchNMSMaterial() since this method is being called from matchNMSMaterial() itself and will cause a StackOverflowError.
        return DUPLICATED.contains(name);
    }

    /**
     * Gets the NMSMaterial based on the material's ID (Magic Value) and data value.<br>
     ...
     * @return a parsed NMSMaterial with the same ID and data value.
     * @see #matchNMSMaterial(ItemStack)
     * @since 2.0.0
    ...
    @Deprecated
    public static Optional<NMSMaterial> matchNMSMaterial(int id, byte data) {
        if (id < 0 || id > MAX_ID || data < 0) return Optional.empty();
        for (NMSMaterial materials : VALUES) {
    ...
    /**
     * NMSMaterial Paradox (Duplication Check)
     ...
     * <p>
     * A solution for NMSMaterial Paradox.
     ...
     * <b>Example:</b>
     * <blockquote><pre>
     *     NMSMaterial material = {@link #matchNMSMaterial(ItemStack)};
    ...
    public boolean isOneOf( Collection<String> materials) {
        ...
        for (String comp : materials) {
            ...
            Optional<NMSMaterial> xMat = matchNMSMaterial(comp);
    ...
    /**
     * Checks if the given material name matches any of this NMSMaterial's legacy material names.
     * All the values passed to this method will not be null or empty and are formatted correctly.
     *
    ...
    /**
     * Parses an item from this NMSMaterial.
    ...
    private boolean isDuplicated() {
        switch (this.name()) {
            ...
            case "NETHER_BRICK":
                ...
                // Illegal Elements
                // Since both 1.12 and 1.13 have <type>_DOOR NMSMaterial will use it
                // for 1.12 to parse the material, but it needs <type>_DOOR_ITEM.
                // We'll trick NMSMaterial into thinking this needs to be parsed
